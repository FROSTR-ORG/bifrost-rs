...
```bash
tail -n 100 <log-directory>/bifrostd-alice.log
```
...
If running in local development mode and intentionally disabling auth for all methods, set:
...
```bash
scripts/devnet.sh status
tail -n 100 <log-directory>/relay.log
```
...
- failing command
- exact stderr output
- relevant log tail under log directory
- current config in runtime config directory (`daemon-*.json`, redacting secrets)
- audit evidence path if applicable
