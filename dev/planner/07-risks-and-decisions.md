...
| `R-006` | Local daemon RPC misuse if operators intentionally run with `auth.insecure_no_auth=true` or weak socket isolation. | high | low | Fail closed by default (`auth.token` required unless explicitly insecure), enforce socket mode `0600`, and keep insecure mode documented as dev-only accepted risk. | runtime | open |
...
| `R-008` | Transitive dependency advisory `RUSTSEC-2023-0089` (`atomic-polyfill`) remains until Frost dependency chain updates upstream. | medium | medium | Keep accepted-risk documented with explicit owner/review date; run `cargo audit` each release and close risk immediately once dependency chain drops advisory. | security/runtime | open |
...
| `D-010` | 2026-02-27 | Audit found local daemon risks: fail-open auth defaults, unbounded RPC frame reads, and implicit socket perms. | Harden daemon defaults: require token unless `insecure_no_auth=true`, cap RPC request lines to `64 KiB`, and enforce Unix socket mode `0600` at bind time. | Stronger local control-plane safety and explicit dev-mode risk acknowledgment. |
