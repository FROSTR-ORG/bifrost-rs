...
| Nonce exhaustion + replenish race | core/node | M3/M6 | done | `crates/bifrost-node/tests/adversarial.rs::adversarial_nonce_exhaustion_fails_second_sign` plus indexed batch nonce-claim path in `bifrost-node::node::tests::sign_batch_flow_returns_signatures`. |
...
| Nonce claim-once and no-reuse guarantees | core/node | M1/M5 | done | `bifrost-core::nonce::tests::outgoing_signing_nonces_are_single_use` + atomic multi-claim path exercised by `bifrost-node::node::tests::sign_batch_flow_returns_signatures`. |
| Batch hash-index to nonce binding correctness | core/node | M1/M5 | done | `bifrost-node::node::tests::handle_incoming_sign_rejects_malformed_nonce_hash_index` + `bifrost-core::sign` indexed verify/combine tests. |
| Stateless sign/ECDH protocol helper flow | frostr-utils | M14 | done | `frostr-utils::protocol::tests::{stateless_sign_flow_roundtrip,stateless_ecdh_flow_roundtrip}`. |
...
| Fixture corpus for RPC/session/onboard edge-cases | codec | M14 | done | `crates/bifrost-codec/tests/fixtures/rpc_parse_matrix.json` + `cargo test -p bifrost-codec --test fixture_matrix --offline`. |
| Multi-node fault-injection integration coverage | node/runtime | M14 | done | `cargo test -p bifrost-node --test fault_injection --offline` (`partial quorum timeout`, `delay timeout`, `churn recovery`, delayed ECDH success). |
| CI coverage report artifact (`llvm-cov`) | infra/qa | M14 | done | `.github/workflows/ci.yml` `coverage` matrix task produces `artifacts/lcov.info` + `artifacts/coverage-summary.txt` (`coverage-report` upload). |
| Dev relay BIP-340 verification and filter matching | relay | M9 | done | `cargo test -p bifrost-devtools --offline` (`verify_event_accepts_valid_signature`, `filter_matches_kind_author_and_tag`). |
...
| `bifrost-tui` devnet e2e RPC method exercise | runtime/tui | M9/M10/M14 | done | `scripts/test-tui-e2e.sh` (real devnet daemons + scripted TUI commands for `health/status/events/use/ping/echo/onboard/sign/ecdh`; asserts success-path outputs). |
| `bifrost-tui` UX regression guardrails (peer selectors + nonce-health status formatting + testing ergonomics) | runtime/tui | M9/M14 | done | `cargo test -p bifrost-tui --offline` (`resolve_peer_selector_supports_index_alias_and_prefix`, `format_status_lines_includes_nonce_columns`, `parse_sign_digest_supports_text_and_hex_modes`, `resolve_echo_target_prefers_explicit_selector_and_falls_back_to_active`, `resolve_echo_target_requires_active_peer_for_shorthand`) + `scripts/test-tui-e2e.sh`; includes alias selectors (`alice,bob,carol`), `use <peer>` target selection, and text-first `sign` flow. |
| Peer policy enforcement + sync + randomized eligible selection | node/runtime/rpc | M14 | done | `cargo test -p bifrost-node --offline` (unit + integration suites), `cargo test -p bifrostd --offline`, `cargo test -p bifrost-cli --test e2e_cli --offline`, `scripts/test-node-e2e.sh`, `scripts/test-tui-e2e.sh`; covers granular `request/respond` policy enforcement, ping-scoped profile exchange, and sign/ECDH eligible peer selection gates. |
