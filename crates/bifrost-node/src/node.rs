...
use crate::types::{BifrostNodeConfig, BifrostNodeOptions, PeerData, PeerPolicy, PeerStatus};
...
impl<T: Transport, C: Clock> BifrostNode<T, C> {
    ...
    pub async fn ping(&self, peer: &str) -> NodeResult<PingPayload> {
        ...
        let RpcPayload::Ping(wire) = res.envelope.payload else {
            return Err(NodeError::InvalidResponse);
        };

        let parsed: PingPayload = wire
            .try_into()
    ...
    pub async fn onboard(&self, peer: &str) -> NodeResult<OnboardResponse> {
        ...
        let RpcPayload::OnboardResponse(wire) = res.envelope.payload else {
            return Err(NodeError::InvalidResponse);
        };

        let onboard: OnboardResponse = wire
            .try_into()
            .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;
    ...
    pub async fn sign(&self, _message: [u8; 32]) -> NodeResult<[u8; 64]> {
        ...
        let self_signing_nonces = {
            ...
            pool.take_outgoing_signing_nonces(self.share.idx, self_nonce.code)
                .ok_or(NodeError::Core(
                    "failed to retrieve self signing nonces".to_string(),
                ))?
        };
        ...
        for msg in responses {
            let RpcPayload::SignResponse(wire) = msg.envelope.payload else {
                return Err(NodeError::InvalidResponse);
            };
            let pkg: bifrost_core::types::PartialSigPackage = wire
                .try_into()
                .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;
    ...
    pub async fn ecdh(&self, pubkey: [u8; 33]) -> NodeResult<[u8; 32]> {
        ...
        for msg in responses {
            let RpcPayload::Ecdh(wire) = msg.envelope.payload else {
                return Err(NodeError::InvalidResponse);
            };
            let pkg: EcdhPackage = wire
                .try_into()
                .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;
            pkgs.push(pkg);
        }
    ...
    pub async fn handle_incoming(&self, msg: IncomingMessage) -> NodeResult<()> {
        ...
        let payload = match msg.envelope.payload {
            ...
            RpcPayload::Ping(wire) => {
                let ping: PingPayload = wire
                    .try_into()
                    .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;

                let peer_idx = self.member_idx_by_peer_pubkey(&sender)?;
                let mut pool = self
                    .pool
                    .lock()
                    .map_err(|_| NodeError::Core("nonce pool poisoned".to_string()))?;
            ...
            RpcPayload::OnboardRequest(wire) => {
                let request: bifrost_core::types::OnboardRequest = wire
                    .try_into()
                    .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;
            ...
            RpcPayload::Sign(wire) => {
                let session: bifrost_core::types::SignSessionPackage = wire
                    .try_into()
                    .map_err(|e: bifrost_codec::CodecError| NodeError::Core(e.to_string()))?;
                self.validate_sign_session(&session)?;
                let requester_idx = self.member_idx_by_peer_pubkey(&sender)?;
    ...
    fn request_id(&self) -> String {
        format!("{}-{}", self.clock.now_unix_seconds(), self.share.idx)
    }
...
mod tests {
    ...
    use bifrost_codec::wire::SignSessionPackageWire;
    ...
    use crate::types::BifrostNodeOptions;
    ...
    impl Transport for TestTransport {
        ...
        async fn request(
            ...
            Err(TransportError::Backend(
                "request not used in this test".to_string(),
            ))
        }
